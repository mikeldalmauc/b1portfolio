module Views.Componentes exposing (..)
...
footerNavigation d prev next =
    let
        rowAttrs =
            \route ->
                [ width fill, height fill, pointer, inFront <| Element.link [ width fill, height fill ] { url = Route.encode route, label = none } ]
                    ++ (if d.width < 700 then
        ...
        textAttrs =
            [ Font.color linkBlue, Font.underline ]
                ++ (if d.width < 700 then
                        montserratSemi
        ...
        prevLink =
            case prev of
                Just ( r, t ) ->
                    wrappedRow
                        (rowAttrs r)
                        [ leftArrowSvg [ height <| px 20 ] grisclaroHex
            ...
            case next of
                Just ( r, t ) ->
                    wrappedRow
                        ...
                        , rightArrowSvg [ height <| px 20 ] grisclaroHex
                        ]
    ...
    Element.wrappedRow
        [ spacing 20
        , paddingEach { top = 150, bottom = 20, left = 20, right = 20 } -- paddingXY 0 20
        , centerX
